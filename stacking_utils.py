...
def StackTwoShapes(shape_colors_1, shape_colors_2):
    ...
    for i in range(np.shape(layered)[0]):
        if np.array_equal(shape_colors_2[i][:], np.asarray(color_key['hole'])):
            layered[i][:] = shape_colors_1[i][:]
